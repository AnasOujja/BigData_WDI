...
mkdir -p certs/{ca,os-dashboards,logstash}
...
# il faut que le repertoire certs appartienne au user 1000 car c'est l'ID du user opensearch das les containers 
sudo chown -R $(awk -F':' '$3 == 1000{ print $1 }' /etc/passwd):$(awk -F':' '$3 == 1000{ print $1 }' /etc/passwd) certs
